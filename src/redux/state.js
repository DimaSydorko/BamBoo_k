let store = {
  ...
  addPost () {
    let newPost = {
      id: "5",
      text: store.state.dataProfilePage.NewPostInput,
      likes: "0"
    }
    store.state.dataProfilePage.PostState.push(newPost)
    store.state.dataProfilePage.NewPostInput = ''
    store.rerenderEntireTree(store.state)
  },

  updateNewPostInput(newText) {
    store.state.dataProfilePage.NewPostInput = newText
    store.rerenderEntireTree(store.state)
  },

  subscribe(check) {
    store.rerenderEntireTree = check
  }
}
